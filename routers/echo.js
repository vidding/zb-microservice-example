...
async function echo(ctx) {
    ...
    ctx.res = { message: 'Helo '.concat(ctx.req.message) }
   }
   ...
   async function serverStreamingEcho(ctx) {
    ...
    // 使用 全双工对象流模式
      ...
      myTransform.write({message: '111'})
      setTimeout(function() {
        myTransform.write({message: '222'})
      }, 1000)
      setTimeout(function() {
        myTransform.write({message: '333'})
      }, 2000)
      setTimeout(function() {
        myTransform.write({message: '444'})
      }, 5000)
   }
